...
import requests  # added: posts parsed readings to the Flask API
...
def process_chunk(chunk: bytes) -> None:
    ...
    try:
        ...
        # added: post to Flask API so data reaches MongoDB and the frontend dashboard
        _post_reading(sensor_msg)
    except ValueError:
        print("Wrong number of bytes")


# converts a SensorMessage and POSTs it to the Flask API
def _post_reading(sensor_msg: SensorMessage) -> None:
    ...
    reading = {
        ...
        "gas_raw":        int(sensor_msg.gas_sensor),
        "pack_voltage":   round(float(sensor_msg.pack_voltage), 4),
        "thermal_row":    sensor_msg.row_sequence,
        "thermal_pixels": sensor_msg.payload,
    }
    try:
